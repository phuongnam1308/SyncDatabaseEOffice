  ...
  async syncFromTempToMain() {
    ...
    try {
      ...
      const records = await pool.request().query(`
        SELECT * FROM ${process.env.NEW_DB_NAME}.dbo.incomming_documents2
        ...
        for (const row of batch) {
          try {
            ...
            const exist = await pool.request()
              ...
              .query(`
                SELECT 1 FROM ${process.env.NEW_DB_NAME}.dbo.incomming_documents
            ...
            const sql = `
              INSERT INTO ${process.env.NEW_DB_NAME}.dbo.incomming_documents (${fields.join(', ')})
              VALUES (${placeholders})
            `;
