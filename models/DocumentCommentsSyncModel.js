  ...
  async countUnsynced() {
    try {
      const query = `
        ...
        FROM ${process.env.NEW_DB_NAME}.dbo.document_comments2 dc2
        WHERE NOT EXISTS (
          SELECT 1
          FROM ${process.env.NEW_DB_NAME}.dbo.document_comments dc
          WHERE dc.id = dc2.id
        )
      `;
  ...
  async getBatchWithDocumentId(offset = 0, limit = 100) {
    try {
      const query = `
        ...
        FROM ${process.env.NEW_DB_NAME}.dbo.document_comments2 dc2
        LEFT JOIN ${process.env.NEW_DB_NAME}.dbo.outgoing_documents_sync od 
