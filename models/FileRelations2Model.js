...
class FileRelations2Model extends BaseModel {
  ...
  async getFromOldDbPaged(page = 0, pageSize = 1000) {
    try {
      ...
      logger.info(`Chạy query getFromOldDbPaged trên ${process.env.NEW_DB_NAME}: page=${page}, offset=${offset}, pageSize=${pageSize}`);
  ...
  async countOldDb() {
    try {
      logger.info(`Đếm bản ghi bảng old: ${this.oldSchema}.${this.oldTable} trong ${process.env.NEW_DB_NAME}`);
