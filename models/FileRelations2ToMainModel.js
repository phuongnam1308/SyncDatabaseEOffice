...
class FileRelations2ToMainModel {
  async migrateAll() {
    const sql = `
      INSERT INTO ${process.env.NEW_DB_NAME}.dbo.file_relations (
        object_type,
        object_id,
        file_id,
        created_at,
  ...
  async countSource() {
    const rs = await db.query(
      'SELECT COUNT(*) AS total FROM ${process.env.NEW_DB_NAME}.dbo.file_relations2'
    );
  ...
  async countTarget() {
    const rs = await db.query(
      'SELECT COUNT(*) AS total FROM ${process.env.NEW_DB_NAME}.dbo.file_relations'
    );
    return rs.recordset[0].total;
  }
}
