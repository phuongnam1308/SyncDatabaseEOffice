...
class IncomingDocumentModel extends BaseModel {
  ...
  async checkExistsInMainTable(id_bak, doc_id) {
    let query = `SELECT 1 FROM ${process.env.NEW_DB_NAME}.dbo.incomming_documents WHERE `;
    const params = {};
  ...
  async insertToMainTable(data) {
    ...
    const query = `
      INSERT INTO ${process.env.NEW_DB_NAME}.dbo.incomming_documents 
      (${fields.join(', ')}) 
      VALUES (${values})
    `;
  ...
  async markAsSynced(identifier) {
    const query = `
      UPDATE ${process.env.NEW_DB_NAME}.dbo.incomming_documents2 
  ...
  async countTempTable() {
    return await this.count('incomming_documents2', `${process.env.NEW_DB_NAME}.dbo`, false);
  }

  async countMainTable() {
    return await this.count('incomming_documents', `${process.env.NEW_DB_NAME}.dbo`, false);
  }

  async countSynced() {
    const query = `SELECT COUNT(*) as cnt FROM ${process.env.NEW_DB_NAME}.dbo.incomming_documents2 WHERE tb_update = 1`;
