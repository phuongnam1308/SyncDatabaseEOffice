...
class IncomingDocumentSyncModel {
  ...
  async connect() {
    if (!this.pool) {
      ...
      logger.info('[SyncModel] Kết nối DB mới OK');
    }
    return this.pool;
  }
  ...
  async getStatistics() {
    ...
    const [sourceRes, destRes] = await Promise.all([
      pool.request().query(`SELECT COUNT(*) as cnt FROM ${process.env.NEW_DB_NAME}.dbo.incomming_documents2`),
      pool.request().query(`SELECT COUNT(*) as cnt FROM ${process.env.NEW_DB_NAME}.dbo.incomming_documents`)
    ]);
