...
class TaskDeleteModel extends BaseModel {
  constructor() {
    super();
    this.oldTable = 'TaskVBDenDelete';
    this.oldSchema = 'dbo';
    this.newTable = 'task2';
    this.newSchema = 'dbo';
  }
  ...
  async getAllFromOldDb() {
    try {
      ...
      logger.error('Lỗi lấy danh sách TaskVBDenDelete từ DB cũ:', error);
      throw error;
    }
  }
  ...
  async findByBackupId(backupId) {
    try {
      ...
      logger.error('Lỗi tìm task delete theo id_taskBackups:', error);
      throw error;
    }
  }
  ...
  async insertToNewDb(data) {
    try {
      logger.info('Dữ liệu sắp insert vào task2:', JSON.stringify(data, (k, v) => v instanceof Date ? v.toISOString() : v, 2));
      ...
      logger.error('Lỗi insert vào task2:', error);
      throw error;
    }
  }
...
module.exports = TaskDeleteModel;