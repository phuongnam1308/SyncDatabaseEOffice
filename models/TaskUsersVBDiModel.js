// models/TaskUsersVBDiModel.js
const BaseModel = require('./BaseModel');
const logger = require('../utils/logger');
const mssql = require('mssql');

class TaskUsersVBDiModel extends BaseModel {
  constructor() {
    super();
    this.oldTable = 'TaskVBDiPermission';
  ...
  async getAllFromOldDb() {
    try {
      ...
      logger.error('Lỗi lấy dữ liệu TaskVBDiPermission + UserField:', error);
      throw error;
    }
  }
  ...
  async countOldDb() {
    const query = `
      SELECT COUNT(*)
      FROM ${this.oldSchema}.TaskVBDiPermission p
  ...
  async findByBackupKeys(taskIdBak, userIdBak, role) {
    try {
      ...
      logger.error('Lỗi tìm task_users2 theo id_task_bak + userId_bak + role (VBDi):', error);
      throw error;
    }
  }
  ...
  async insertToNewDb(data) {
    try {
      ...
      logger.error('Lỗi insert vào task_users2 (VBDi):', error);
      throw error;
    }
  }
...
module.exports = TaskUsersVBDiModel;
