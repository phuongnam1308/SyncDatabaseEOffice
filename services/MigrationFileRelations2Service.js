...
class MigrationFileRelations2Service {
  ...
  async getStatistics() {
    try {
      ...
      return {
        source: { database: process.env.NEW_DB_NAME, schema: 'dbo', table: 'files2', count: oldCount },
        destination: { database: process.env.NEW_DB_NAME, schema: 'dbo', table: 'file_relations2', count: newCount },
        migrated: newCount,
