// services/MigrationTaskService.js
const TaskModel = require('../models/TaskModel');
const { tableMappings } = require('../config/tablesTask');
...
class MigrationTaskService {
  ...
  async initialize() {
    try {
      ...
      logger.info('MigrationTaskService đã được khởi tạo');
    } catch (error) {
      logger.error('Lỗi khởi tạo MigrationTaskService:', error);
      throw error;
    }
  }
  ...
  async migrateTaskRecords() {
    ...
    logger.info('=== BẮT ĐẦU MIGRATION TASKVBDEN → TASK2 ===');

    try {
      const config = tableMappings.task;
          ...
          try {
            ...
            switch (oldRecord.TrangThai) {
              ...
              case 'Hủy':
                ...
                break;
              default:
                newRecord.status = 1;  // Default
            }

            // Map Priority sang priority
      ...
      logger.info('=== HOÀN THÀNH MIGRATION TASK2 ===');
      ...
      logger.error('Lỗi trong quá trình migration Task2:', error);
      throw error;
    }
  }
  ...
  async getStatistics() {
    try {
      ...
      logger.error('Lỗi lấy thống kê Task2:', error);
      throw error;
    }
  }
...
module.exports = MigrationTaskService;