...
class IncomingSyncService {
  ...
  async preview() {
    const sql = `
      ...
      FROM ${process.env.NEW_DB_NAME}.dbo.${this.srcTable} s
      WHERE NOT EXISTS (
        SELECT 1 FROM ${process.env.NEW_DB_NAME}.dbo.${this.destTable} d
  ...
  async sync(batchSize = 1000) {
    /** ---------- INSERT ---------- */
    const insertSql = `
      INSERT INTO ${process.env.NEW_DB_NAME}.dbo.${this.destTable} (
        document_id,
        status_code,
        created_at,
        updated_at,
    ...
    const updateSql = `
      ...
      FROM ${process.env.NEW_DB_NAME}.dbo.${this.destTable} d
      JOIN ${process.env.NEW_DB_NAME}.dbo.${this.srcTable} s
