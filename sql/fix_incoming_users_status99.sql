...
BEGIN TRY
    -- -------------------------------------------------------------------------------------
    -- BƯỚC 1: Lập bảng tạm ánh xạ User 1-1 chắc chắn (#UserMapping)
    -- -------------------------------------------------------------------------------------
    ...
    -- CHỈ GIỮ LẠI BẢN GHI KHỚP 1-1 AN TOÀN:
    Valid1To1Matches AS (
        SELECT 
    ...
    SELECT @SingleCount = COUNT(1) FROM #UserMapping;

    PRINT N'-> Đã lập bản đồ ánh xạ 1-1 chắc chắn cho ' + CAST(@SingleCount AS NVARCHAR(10)) + N' user.';

    -- -------------------------------------------------------------------------------------
    -- BƯỚC 2: Cập nhật dbo.incomming_documents (Các trường thuộc Văn bản đến)
    -- -------------------------------------------------------------------------------------
    ...
    BEGIN
        ...
        END
        PRINT N'-> Đã cập nhật dbo.incomming_documents';
    END

    -- -------------------------------------------------------------------------------------
    -- BƯỚC 3: Cập nhật dbo.audit (Văn bản đến)
    -- -------------------------------------------------------------------------------------
    ...
    BEGIN
        ...
        PRINT N'-> Đã cập nhật dbo.audit';
    END

    -- -------------------------------------------------------------------------------------
    -- BƯỚC 4: Cập nhật dbo.incomming_assignment (Xử lý trùng PK)
    -- -------------------------------------------------------------------------------------
    ...
    BEGIN
        ...
        PRINT N'-> Đã cập nhật dbo.incomming_assignment';
    END

    -- -------------------------------------------------------------------------------------
    -- BƯỚC 5: Cập nhật dbo.incomming_current_state
    -- -------------------------------------------------------------------------------------
    ...
    BEGIN
        ...
        PRINT N'-> Đã cập nhật dbo.incomming_current_state';
    END

    -- -------------------------------------------------------------------------------------
    -- BƯỚC 6: Cập nhật dbo.document_comments (Nếu có)
    -- -------------------------------------------------------------------------------------
    ...
    BEGIN
        ...
        END
        PRINT N'-> Đã cập nhật dbo.document_comments';
    END

    COMMIT TRANSACTION;
    PRINT N'✅ ĐÃ CẬP NHẬT HOÀN TẤT VÀ CHÍNH XÁC DÀNH CHO VĂN BẢN ĐẾN!';
