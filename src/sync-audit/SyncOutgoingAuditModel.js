...
class SyncOutgoingAuditModel extends SyncAuditModel {
  ...
  async _syncToAssignment(audit, auditId, transaction) {
    ...
    try {
      ...
      await this.queryNewDbTx(
        `DELETE FROM ${process.env.NEW_DB_NAME}.dbo.outgoing_assignment  WITH (ROWLOCK, READPAST) 
      ...
      for (const { rec, unit } of allReceivers) {
        ...
        await this.queryNewDbTx(
          `INSERT INTO ${process.env.NEW_DB_NAME}.dbo.outgoing_assignment  WITH (ROWLOCK, READPAST) 
  ...
  async _syncToCurrentState(audit, auditId, transaction) {
    ...
    await this.queryNewDbTx(
      `MERGE ${process.env.NEW_DB_NAME}.dbo.outgoing_current_state  WITH (ROWLOCK, READPAST)  AS tgt
