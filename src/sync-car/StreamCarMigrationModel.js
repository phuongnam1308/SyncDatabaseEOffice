...
class StreamCarMigrationModel extends BaseIncrementalSyncInterface {
  ...
  async getCarByName(name) {
    ...
    const rows = await this.queryNewDb(`SELECT TOP 1 id FROM ${ref} WHERE license_plate = @license_plate`, {
      license_plate: name,
    });
    return rows?.[0] || null;
  }
  ...
  async processRowData(rowData) {
    ...
    const returnTime =
      this.parseDate(rowData.original_modified) ||
      this.parseDate(rowData.original_created) ||
      new Date();
    ...
    // Create audit records for tracking (five records per registration)
  ...
  async ensureDriverExists(id, label) {
    ...
    await this.queryNewDb(`
      INSERT INTO ${ref} (
        ...
        @id, @label, '0908886678', '090967555257', 'taixecongty@gmail.com', 'Địa chỉ công ty',
  ...
  _getDefaultAuditEntries(registrationId, mappedUserId, rowData, now) {
    return [
      ...
      },
      {
        ...
        origin_id: 'wi_1777444413843_6detdw',
        created_by: mappedUserId,
      ...
      },
      {
        ...
        origin_id: 'wi_1777444555706_2p1kch',
        created_by: 'Hệ thống',
        receiver: 'Hệ thống',
      ...
      },
      {
        ...
        origin_id: 'wi_1777531508680_9fouw4',
        created_by: mappedUserId,
