...
class StreamEventMigrationModel extends BaseIncrementalSyncInterface {
  ...
  async ensureStagingTableExists() {
    try {
      ...
      const columnsToAdd = [
          ...
          { name: 'KetThuc', type: 'NVARCHAR(500)' },
          { name: 'DiaDiem', type: 'NVARCHAR(500)' },
  ...
  async getList(lastSyncTime, syncJobId, lastSyncId = 0) {
    ...
    return {
      ...
      sourceLastSyncTime: normalizedLastSyncTime, sourceLastSyncId: normalizedLastSyncId,
  ...
  async processRowData(rowData, { transaction } = {}) {
    ...
    // Mapping Người chủ trì (Chairman/Organizer)
  ...
  async upsertDataToNewDB(rawData, config, externalKeyField, externalKeyValue, transaction) {
    ...
    for (const [newField, valueFn] of Object.entries(defaultValues || {})) {
      ...
      if (!params.hasOwnProperty(newField)) {
        ...
        insertCols.push(`[${newField}]`);
        insertVals.push(`@${newField}`);
        updateSet.push(`[${newField}] = @${newField}`); // Thêm vào update set cho default values nếu cần
      }
    }
