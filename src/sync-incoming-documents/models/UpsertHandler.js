...
class UpsertHandler {
  ...
  async processRecord(oldRecord) {
    ...
    try {
      ...
      try {
        preparedFiles = await this._prepareFilesFromSharePoint_v2(oldRecord);
      } catch (error) {
        logger.error(`[UpsertHandler:Incoming] _prepareFilesFromSharePoint_v2 ID=${id}: ${error.message}`);
        throw error;
      }
  ...
  async _prepareFilesFromSharePoint_v2(oldRecord) {
    const files = String(oldRecord?.Files || '').trim();
    if (!files) return [];

    const baseUrl = String(process.env.BASE_URL || '').replace(/\/$/, '');
    if (!baseUrl) return [];

    const fileExtRegex = /\.(pdf|docx?|xlsx?|pptx?|jpe?g|png|gif|bmp|txt|zip|rar)$/i;
    ...
    try {
      // ============================
      // V2: SharePoint format
      // path|displayName|;#path2|displayName2|
      // ============================
      if (files.includes(';#')) {
        const entries = files
          .split(';#')
        ...
        for (const entry of entries) {
          ...
          if (!fileExtRegex.test(normalizedPath)) continue;
          if (uniquePaths.has(normalizedPath)) continue;

          uniquePaths.add(normalizedPath);
          filesToPath.push(normalizedPath);
        }
      } else {
        // ============================
        // Backward compatible format cũ
        ...
        const firstIsFile = fileExtRegex.test(parts[0]);

        if (firstIsFile) {
          const normalizedPath = parts[0];

          if (
            normalizedPath.includes('/') &&
            !uniquePaths.has(normalizedPath)
          ) {
            uniquePaths.add(normalizedPath);
            filesToPath.push(normalizedPath);
          }
          ...
          for (const name of parts.slice(1)) {
            ...
            const relativePath = directory.endsWith('/')
              ? `${directory}${name}`
              : `${directory}/${name}`;

            if (!fileExtRegex.test(relativePath)) continue;
            if (uniquePaths.has(relativePath)) continue;

            uniquePaths.add(relativePath);
            filesToPath.push(relativePath);
          }
        }
      }

      if (!filesToPath.length) {
        return [];
      }

      // Log when downloading more than 2 files (helpful for debugging large attachments)
      if (filesToPath.length > 2) {
        try {
          const preview = filesToPath.slice(0, 10).join(', ');
          logger.info(`[UpsertHandler:Incoming][prepareFiles] ID=${oldRecord?.ID || ''} downloading ${filesToPath.length} files: ${preview}${filesToPath.length > 10 ? ', ...' : ''}`);
        } catch (err) {
      ...
      const downloadPromises = filesToPath.map(async (relativePath) => {
        try {
          ...
          if (!buffer || !buffer.length) {
            logger.warn(
              `[UpsertHandler:Incoming][prepareFiles] Empty file buffer: ${relativePath}`
            );
            return null;
          }
          ...
          logger.error(
            `[UpsertHandler:Incoming][prepareFiles] Error downloading ${relativePath}: ${err.message}`
          );
      ...
      logger.error(
        `[UpsertHandler:Incoming][prepareFiles] Fatal error: ${err.message}`
      );
  ...
  async _mapAllRawAudits(allRawAudits, documentId, drafter, transaction) {
    ...
    for (const rawAudit of allRawAudits) {
      try {
        ...
        const typeDocument = 'IncomingDocument';
        // Gọi _mapSingleRecord thay vì processSingleRecord để lấy mapped object
        const mapped = await model._mapSingleRecord(rawAudit, documentId, transaction, drafter, typeDocument);
