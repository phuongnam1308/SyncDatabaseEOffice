...
// �?c c?u h�nh t? environment variables (du?c set t? .bat file)
...
for (const arg of args) {
  ...
  if (arg.startsWith('--start=')) {
    startId = parseInt(arg.split('=')[1], 10) || startId;
  }
}
...
async function main() {
  ...
  const loader = new Loader(newPool, oldPool, WORKER_COUNT, rangeSize, startId);
  ...
  console.log(`[CAU HINH] Workers: ${WORKER_COUNT} | RangeSize: ${loader.rangeSize} | StartId: ${loader.startId} | MaxPerWorker: ${maxRecordsPerWorker || 'unlimited'}`);
  console.log('');
  ...
  // Kh?i ch?y d�ng WORKER_COUNT workers song song
  const workers = [];
