...
class Loader extends BaseLoader {
  constructor(newPool, oldPool, workerCount = null, rangeSize = null) {
    super({
    ...
    this.workerCount = workerCount || parseInt(process.env.WORKER_COUNT, 10) || 8;
    this.rangeSize = rangeSize || parseInt(process.env.RANGE_SIZE, 10) || 1250;
  }
  ...
  getWorkerIdRange(workerId, roundIndex = 0) {
    ...
    const idMin = baseMin + shift;
    const idMax = baseMax + shift;
    return { idMin, idMax };
  }
  ...
  async fetchOneFromStaging(
    ...
    const stagingTable =
      this.getStagingTableName(instanceId);

    const { idMin, idMax } =
      this.getWorkerIdRange(
        workerId,
        roundIndex
      );
    ...
    try {
      if (!this.newPool) {
        throw new Error(
          'Loader: newPool chưa được kết nối.'
        );
      }
      ...
      reqClaim.input(
        'owner',
        `pid_${process.pid}_w${workerId}_${instanceId}`
      );
