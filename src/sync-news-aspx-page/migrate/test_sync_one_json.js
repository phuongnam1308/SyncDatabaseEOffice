...
async function testSingleJson() {
  ...
  const jsonFileName =
    'de-cuong-tuyen-truyen-ky-niem-60-nam-hai-quan-viet-nam-anh-hung-07-5-1955-07-5-2015.json';
  const jsonFilePath = path.resolve(
    __dirname,
    '../../../tintucraw/tintuc/json_output',
    jsonFileName,
  );
  ...
  try {
    ...
    const result = await pool
      ...
      .query(
        'SELECT TOP 1 title, content FROM dbo.news_aspx_new_sync WHERE slug = @slug ORDER BY created_at DESC',
      );
